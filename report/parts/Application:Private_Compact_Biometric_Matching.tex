...
In this subsection, we will experimentally evaluate the False Negative Rate (FNR) and False Positive Rate (FPR) with compression. We designed an algorithm that uses the previously defined method to generate \( l \) iterations of \textit{PreHash} and then compresses each hash value generated depending on the value of \(D = d²\). For an image, the algorithm generates a list of the \( l \) iterations of \textit{PreHash} and then applies the \textit{PostHash} algorithm to each \textit{PreHash} output.
