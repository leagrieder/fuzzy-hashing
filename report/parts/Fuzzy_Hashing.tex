...
The computation of \( \mu_{\text{indep}}^{obs} \) is relatively straightforward, having determined the value of \( p^{obs} \), \(\delta_{\text{indep}}^{obs}\) and their respective variances. Moreover, because of independence between \(X\) and \(Y\), the covariances that appear in Equation~\ref{eq:mu_approximation} are \(0\), allowing us to express \( \mu_{\text{indep}}^{obs} \) as:

\[
    \mu_{\text{indep}}^{obs}  \approx 0.01136 = 1.14\%
\]
