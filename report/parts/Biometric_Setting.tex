...
\begin{equation} \label{eq:proba}
    \sigma_{p} \approx 4 \times 10^{-3}
\end{equation}

\begin{equation} \label{eq:proba}
    \sigma²_{p} \approx 2 \times 10^{-5}
\end{equation}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        \hline
        \textbf{} & \text{Mean (\(\delta^{obs}\))} & \text{Variance (\(\sigma²_{\delta}\))} & \text{Standard Deviation (\(\sigma_{\delta}\))} \\
        \hline
        \textbf{Intra-Individual Distribution} & \(4.5\)\% & \(1.1 \times 10^{-4}\) & \(1.0 \times 10^{-2}\) \\
        \hline
        \textbf{Inter-Individual Distribution} & \(6\)\% & \(4.6 \times 10^{-5}\) & \(6.8 \times 10^{-3}\) \\
        \hline
    \end{tabular}
\caption{Comparison of Distributions: Mean, Variance, and Standard Deviation for $\delta_{same}$ and $\delta_{diff}$}
\end{table}
...
\begin{enumerate}
    ...
    \item When considering the datasets from Both Cameras combined, the probability \(p^{obs} = Pr[X_i = 1]\) averaged out to \(0.033\), suggesting a \(3.3\)\% likelihood of a pixel depicting a vein across the entire dataset. Moreover, the variance (\( \sigma^{2}_p \)) and the standard deviation (\( \sigma_p \)) for this probability across all images of our dataset provide insights into the distribution's spread and consistency. The observed variance is low at approximately \( 2 \times 10^{-5} \), indicating minimal fluctuation in \( p^{obs} \) values among different feature vectors. This suggests that the feature extraction method is highly reliable and consistent across various different images. Furthermore, the standard deviation, calculated as approximately \( 4 \times 10^{-3} \), reinforces the idea of minimal dispersion around the mean probability of detecting a vein. 
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \[ \sigma_{\delta_{same}} \approx 1.0 \times 10^{-2} \]
        \[ \sigma²_{\delta_{same}} \approx 1.1 \times 10^{-4} \]
        ...
        \[ \sigma_{\delta_{diff}} \approx 6.8 \times 10^{-3} \]
        \[ \sigma²_{\delta_{diff}} \approx 4.6 \times 10^{-5} \]
