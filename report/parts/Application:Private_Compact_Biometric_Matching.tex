...
First, let us define the random variable $H$ as \(H = d_H(Hash_{key}(X), Hash_{key}(Y))\). Since the hash keys are generated in a manner such that each preHash comparison is independent, the Hamming distance $H$ can be modeled as a binomial random variable, \(H \sim Binomial(l, p)\), where $l$ is the total number of iterations and $p = P(Hash_{key_i}(X) \neq Hash_{key_i}(Y)) \leq 1 - \mu_{same}^m$ is the probability of the output of the preHash function for a certain key $key_i$ mismatch. This probability calls for $\mu_{same}$ due to the fact that we are approximating the FNR, which, by definition, is when the samples should match but don't. 
