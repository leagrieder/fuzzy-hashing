...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \[ \delta_{\text{same}} \approx 0.046 \]

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.9\linewidth]{latex-img/delta_same.png}
        ...
        \[ \delta_{\text{diff}} \approx 0.06 \]

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.9\linewidth]{latex-img/delta_diff.png}
...
\begin{itemize}
    \item The entry \(P(\bar{X}_i = 0, \bar{Y}_i = 0)\) corresponds to the probability of both bits being '0'. In the absence of any discrepancies (\(\delta = 0\)), this probability would be simply the product of the individual probabilities, \((1-p)^2\). However, considering that discrepancies do occur with probability \(\delta\), and that there are two distinct ways in which the bits can differ (\(X_i = 0, Y_i = 1\) and \(X_i = 1, Y_i = 0\)), we must adjust for these potential errors. Since these two error scenarios are mutually exclusive and equally likely, we subtract half the error probability, \(\frac{\delta}{2}\), from \((1-p)^2\). Moreover, we approximate the final result to \(1-p-\frac{\delta}{2}\) because p is relatively small. 

    \item The probability \(P(\bar{X}_i = 1, \bar{Y}_i = 1)\) is derived in an analogous manner. The probability is given by \(p-\frac{\delta}{2}\).

    \item The probabilities \(P(\bar{X}_i = 0, \bar{Y}_i = 1)\) and \(P(\bar{X}_i = 1, \bar{Y}_i = 0)\) directly represent the scenarios where the bits differ. As there are only two such cases that contribute to \(\delta\), each is allocated a probability of \(\frac{\delta}{2}\).

\end{itemize}

Utilizing our calculated values for \(\delta_{same}\), \(\delta_{diff}\) and \(\delta_{indep}\), we can determine the corresponding probabilities presented in Table \ref{tab:joint_distribution}. This gives us the following figures:
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        & $\bar{Y}_i = 0$ & $\bar{Y}_i = 1$\\
        \hline
        $\bar{X}_i = 0$ & $1 - p - \frac{\delta_{indep}}{2} = 0.935 $ & $\frac{\delta_{indep}}{2} = 0.032$\\
        \hline
        $\bar{X}_i = 1$ & $\frac{\delta_{indep}}{2} = 0.032 $ & $p - \frac{\delta_{indep}}{2} = 0.001$\\
        \hline
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        & $\bar{Y}_i = 0$ & $\bar{Y}_i = 1$\\
        \hline
        $\bar{X}_i = 0$ & $1 - p - \frac{\delta_{same}}{2} = 0.944 $ & $\frac{\delta_{same}}{2} = 0.023$\\
        \hline
        $\bar{X}_i = 1$ & $\frac{\delta_{same}}{2} = 0.023$ & $p - \frac{\delta_{same}}{2} = 0.01$\\
        \hline
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        & $\bar{Y}_i = 0$ & $\bar{Y}_i = 1$\\
        \hline
        $\bar{X}_i = 0$ & $1 - p - \frac{\delta_{diff}}{2} = 0.937 $ & $\frac{\delta_{diff}}{2} = 0.03$\\
        \hline
        $\bar{X}_i = 1$ & $\frac{\delta_{diff}}{2} = 0.03$ & $p - \frac{\delta_{diff}}{2} = 0.003$\\
        \hline
    \end{tabular}
